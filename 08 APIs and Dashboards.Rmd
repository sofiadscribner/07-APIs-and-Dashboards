...
Databases are a type of server. Instead of querying a database, **APIs** (Application Programming Interfaces) are a *general* way to access any server that provide permission to access data along with throttling what and how much data you can access.
...
Install and load the {qualtRics} package. Remember to call `install.packages()` in the console only!

Still in the console, call the following:
...
We can use **dashboards** to interactively report on key performance indicators, visualizations, and data exploration. We can build dashboards using R Markdown (and Quarto soon). We'll also use [{shiny}](https://shiny.rstudio.com) functions, the industry-standard for webapps, to make the dashboard interactive.

Install the {markdown}, {rmarkdown}, {flexdashboard}, {shiny}, and {rsconnect} packages using the console. Create a new dashboard by creating an **R Markdown** document > **From Template** > **Flex Dashboard**.
...
# Import data (no need to use here::here() this time).
...
**Input functions** make the dashboard interactive. There are many different input functions, but they all include an input ID and an input label.
...
In RStudio on Posit Cloud, do the following in *two* documents.
